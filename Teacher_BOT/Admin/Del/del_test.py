...
#Удаление теста
async def del_test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args or len(args) != 5:
        await update.effective_message.reply_text(
            '✏️ Использование: /del_test "Название дисциплины" "Название курса" "Название темы" "Название урока" "Вопрос"'
        )
        return

    discipline, course, topic, lesson, question = args
    ...
    if not db.is_lesson(discipline, course, topic, lesson):
        ...
        return
    if not db.is_question(discipline, course, topic, lesson, question):
        await update.message.reply_text(f"❌ Вопрос {question} не найден в уроке {lesson} в теме {topic} в курсе {course} и в дисциплине {discipline}.")
        return

    db.del_test(discipline, course, topic, lesson, question)
    await update.message.reply_text(f"✅ Вопрос {question} успешно удален!!!")