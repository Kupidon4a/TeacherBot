...
async def handle_topic_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.get_discipline() == None or user.get_course() == None or user.get_title() != None:
        print(user.get_discipline())
        print(user.get_course())
        print(user.get_topic())
        print(user.get_title())
        
        await query.message.reply_text(f"❌ Ошибка сценария использования. Пожалуйста не нажимайте кнопки не в последнем сообщении от бота! мяу")
        return
